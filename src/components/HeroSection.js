...
const HeroSection = ({ title, subtitle, callToAction }) => {
  return (
    <section className="py-24 md:py-36 px-4 flex flex-col items-center justify-center text-center">
      ...
      <Link
        className="mt-20 px-8 md:px-10 lg:px-12 py-3 md:py-4 lg:py-6 bg-action hover:bg-action/90 rounded-full text-white dark:text-primary-dark text-lg md:text-xl lg:text-2xl font-medium duration-300 transition-all"
        href={"/explore"}
      >
        {callToAction}
      </Link>
